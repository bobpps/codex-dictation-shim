...
beforeEach(async () => {
  if (codexHome) await removeDir(codexHome);
  codexHome = await makeTempDir('shim-codex-');
});
after(async () => {
  if (codexHome) await removeDir(codexHome);
});
...
describe('reading auth.json', () => {
  it('returns the token and account id from a ChatGPT login', async () => {
  ...
  it('re-reads the file on every call so a refresh is picked up', async () => {
    // Codex rewrites auth.json when it refreshes. A value cached at startup is
    // a value that goes stale with no event to notice.
...
describe('credential failures', () => {
  it('names the file and the command when there is no auth.json', async () => {
  ...
  it('explains an API-key login instead of throwing on undefined', async () => {
    // `codex login --with-api-key` writes a different shape with no tokens
    // object. Reading `tokens.access_token` off it is a TypeError; saying which
    // login the endpoint needs is an answer.
    await writeFile(
  ...
  it('rejects an auth_mode that is not chatgpt even when tokens are present', async () => {
  ...
  it('says how long ago the token died, and what refreshes it', async () => {
    ...
    await assert.rejects(readAuth({ codexHome }), (error) => {
      ...
      // The ten-day TTL plus a CLI that only refreshes while it runs is exactly
      // how a machine ends up with a token that died days before anyone looked.
      assert.match(error.hint, /idle machine/);
      return true;
    });
  });

  it('reports unreadable JSON rather than a parse stack', async () => {
...
describe('degraded but usable credentials', () => {
  it('warns about a missing account id instead of refusing', async () => {
  ...
  it('warns when the token is not a readable JWT and proceeds anyway', async () => {
  ...
  it('warns when the token carries no exp claim', async () => {
...
describe('describeAuth for /health', () => {
  it('reports expiry without ever exposing the token', async () => {
    await writeAuthFile(codexHome, { accountId: 'acct-42' });
  ...
  it('turns a failure into a readable field instead of an exception', async () => {
    const report = await describeAuth({ codexHome });
