...
before(async () => {
  dir = await makeTempDir('shim-recordings-');
});
after(async () => {
  await removeDir(dir);
});
...
describe('picking a recording', () => {
  it('takes the newest wav by mtime, not by name', async () => {
    ...
    assert.equal(claimed.path, expected);
  });

  it('ignores files that are not wav', async () => {
...
describe('guard: freshness', () => {
  it('refuses a recording older than MAX_AGE_SEC', async () => {
    // Handy writes the WAV before post-processing runs, so an old file means
    // this request is not about it — most likely audio retention is off and the
    // newest file is from some earlier dictation entirely.
  ...
  it('accepts a recording inside the window', async () => {
    await writeRecording(dir, 'handy-recent.wav', { ageSec: 59 });
    assert.ok((await claim()).path.endsWith('handy-recent.wav'));
  });
});

describe('guard: still being written', () => {
  it('waits for a growing file and accepts it once it stops', async () => {
    ...
    // Settled means the size stopped moving, so what was measured is the whole
    // file — the point of the guard is that a truncated WAV never goes out.
    assert.ok(settled.size >= 1024);
  });

  it('gives up rather than sending a truncated recording', async () => {
  ...
  it('costs no waiting when the file finished writing a while ago', async () => {
    // The normal path: actions.rs awaits and verifies the WAV before calling
    // post-processing, so by the time the shim is asked the file is long done.
    // If this guard cost its quiet window every time, it would be pure latency
    // added to every single dictation.
...
describe('guard: deduplication', () => {
  it('refuses the same recording twice', async () => {
    // Without this, a failure on the second dictation resends the first one's
    // audio and pastes the first one's words. Nothing about the result looks
    // wrong, which is what makes it worth a hard refusal.
  ...
  it('treats a re-recorded file at the same path as a new recording', async () => {
  ...
  it('claims before the caller does anything, so a retry cannot reuse the audio', async () => {
    await writeRecording(dir, 'handy-1.wav', { ageSec: 1 });
    const claimed = await claim();
  ...
  it('lets a newer recording through while the previous one stays claimed', async () => {
...
describe('guard: not a wav at all', () => {
  it('refuses a file too small to hold a wav header', async () => {
